...
if custom_se_choice:
    SE_of_concern  = st.number_input('**:blue[Systematic error (%) of concern]**', min_value=0.0, max_value=33.0, value = SE_of_concern_maxenuf, step = 0.01)
else:
...
st.markdown(f"""   
                    ...
                    | **:black[Probability of generating a result that exceeds the TEa during in-control state]** | **{ '<0.000001' if p_TEa_exceed_in_control < 0.000001 else format(p_TEa_exceed_in_control, '.6f')}** |
                    | **:black[Probability of generating a result that exceeds the TEa when an error exists]** | **{round(p_TEa_exceed_out_control, 4)}** |
                    | **:black[Increase in the probability of generating a result that exceeds the TEa due to the presenece of an error]** | **{round(delta_p_exceed_TEa, 4)}** |
